...
export function Carousel({
  ...
  // Enhanced error handling and performance monitoring state
  const [imageErrors, setImageErrors] = useState<Map<number, { count: number; lastError: string }>>(new Map());
  const [retryAttempts, setRetryAttempts] = useState<Map<number, number>>(new Map());
  const [performanceMetrics, setPerformanceMetrics] = useState<Map<number, { loadStart: number; loadEnd?: number; size?: number }>>(new Map());
  ...
  const updateResolvedTheme = useCallback(() => {
    const newTheme = theme === 'system' ? detectSystemTheme() : theme;
    setResolvedTheme(newTheme);
    trackUserInteraction('theme_changed', { theme: newTheme, userPreference: theme });
  }, [theme, detectSystemTheme, trackUserInteraction]);
  ...
  const scrollPrev = useCallback(() => {
    if (emblaApi) {
      ...
      trackUserInteraction('navigate_previous');
    }
  }, [emblaApi, triggerButtonAnimation, trackUserInteraction]);

  const scrollNext = useCallback(() => {
    if (emblaApi) {
      emblaApi.scrollNext();
      triggerButtonAnimation('next');
      trackUserInteraction('navigate_next');
    }
  }, [emblaApi, triggerButtonAnimation, trackUserInteraction]);
  ...
  const scrollTo = useCallback((index: number) => {
    if (emblaApi) {
      ...
      trackUserInteraction('navigate_thumbnail', { target_index: index });
    }
  }, [emblaApi, triggerButtonAnimation, trackUserInteraction]);
  ...
  const togglePlayPause = useCallback(() => {
    ...
    trackUserInteraction(isPlaying ? 'slideshow_pause' : 'slideshow_play');
  }, [isPlaying, emblaApi, images.length, triggerButtonAnimation, trackUserInteraction]);
  ...
  const trackUserInteraction = useCallback((action: string, data?: any) => {
    ...
    console.log('Carousel Interaction:', { action, selectedIndex, zoomLevel, isMobile, ...data });
  }, [selectedIndex, images.length, zoomLevel, isMobile]);

  // Performance monitoring
  const trackImagePerformance = useCallback((index: number, event: 'start' | 'end' | 'error', data?: any) => {
    setPerformanceMetrics(prev => {
      const newMetrics = new Map(prev);
      const current = newMetrics.get(index) || { loadStart: Date.now() };
      
      switch (event) {
        case 'start':
          newMetrics.set(index, { loadStart: Date.now() });
          break;
        case 'end':
          newMetrics.set(index, { ...current, loadEnd: Date.now(), size: data?.size });
          break;
        case 'error':
          newMetrics.set(index, { ...current, loadEnd: Date.now() });
          break;
      }
  ...
  const handleImageError = useCallback((index: number, error?: string) => {
    ...
    const errorInfo = imageErrors.get(index) || { count: 0, lastError: '' };
    
    setImageErrors(prev => {
      const newErrors = new Map(prev);
      newErrors.set(index, {
        count: errorInfo.count + 1,
        lastError: error || 'Failed to load image'
      });
      return newErrors;
    });

    trackImagePerformance(index, 'error', { error, retryCount: currentRetries });
    trackUserInteraction('image_error', { index, error, retryCount: currentRetries });
    ...
    if (currentRetries < maxRetryAttempts) {
      setTimeout(() => {
        setRetryAttempts(prev => {
          ...
          newRetries.set(index, currentRetries + 1);
          return newRetries;
        });
        ...
        if (img) {
          ...
          img.src = '';
          setTimeout(() => {
            img.src = originalSrc + (originalSrc.includes('?') ? '&' : '?') + `retry=${currentRetries + 1}`;
          }, 100);
        }
      }, Math.pow(2, currentRetries) * 1000); // Exponential backoff
    } else {
      setErrorRecoveryMode(true);
    }
  }, [retryAttempts, imageErrors, trackImagePerformance, trackUserInteraction]);
  ...
  const handleImageLoadStart = useCallback((index: number) => {
    ...
    // Show skeleton for slower loads
    ...
    setImageLoadingStates(prev => {
      const newStates = new Map(prev);
      newStates.set(`timer-${index}` as any, skeletonTimer as any);
      return newStates;
    });

    trackImagePerformance(index, 'start');
    ...
    const progressInterval = setInterval(() => {
      setImageLoadProgress(prev => {
        ...
        newProgress.set(index, Math.min(90, current + Math.random() * 15));
        return newProgress;
      });
    }, 100);
  ...
  const handleImageLoad = useCallback((index: number) => {
    ...
    setImageLoadingStates(prev => {
      const newStates = new Map(prev);
      const timer = newStates.get(`timer-${index}` as any);
      if (timer) {
        clearTimeout(timer as any);
        newStates.delete(`timer-${index}` as any);
      }
    ...
    setRetryAttempts(prev => {
      const newRetries = new Map(prev);
      newRetries.delete(index);
      return newRetries;
    });

    trackImagePerformance(index, 'end');
    trackUserInteraction('image_loaded', { index, loadTime: performanceMetrics.get(index) });
  }, [trackImagePerformance, trackUserInteraction]);

  // Error recovery function
  const retryFailedImage = useCallback((index: number) => {
    setImageErrors(prev => {
      const newErrors = new Map(prev);
      newErrors.delete(index);
      return newErrors;
    ...
    trackUserInteraction('manual_retry', { index });
  }, [handleImageLoadStart, trackUserInteraction]);
  ...
  const updateUrlState = useCallback(() => {
    ...
    trackUserInteraction('url_updated', { index: selectedIndex, zoom: zoomLevel, metadata: showMetadataOverlay });
  }, [isOpen, selectedIndex, isZoomed, zoomLevel, showMetadataOverlay, trackUserInteraction]);

  const parseUrlState = useCallback(() => {
    if (typeof window === 'undefined') return null;
    
    const url = new URL(window.location.href);
    const carouselOpen = url.searchParams.get('carousel') === 'true';
    const imageIndex = parseInt(url.searchParams.get('image') || '0');
    const zoomParam = url.searchParams.get('zoom');
    const metadataParam = url.searchParams.get('metadata') === 'true';
    
    return {
      shouldOpen: carouselOpen,
      imageIndex: isNaN(imageIndex) ? 0 : Math.max(0, Math.min(imageIndex, images.length - 1)),
      zoom: zoomParam ? parseFloat(zoomParam) : 1,
      showMetadata: metadataParam
    };
  }, [images.length]);
  ...
  useEffect(() => {
    ...
    const updateUrl = () => {
      ...
      params.set('image', selectedIndex.toString());
      if (zoomLevel > 1) {
  ...
  }, [selectedIndex, zoomLevel, isOpen]);
            ...
            {images.map((image, index) => (
              <div key={image.id} className="flex-[0_0_100%] min-w-0 relative">
                <div className="flex items-center justify-center w-full h-full">
                  ...
                  {imageErrors.get(index) && (retryAttempts.get(index) || 0) >= maxRetryAttempts && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                      <div className="flex flex-col items-center gap-4 text-center p-6">
                        ...
                        <p className={cn(
                          "text-white/70 max-w-xs",
                          isMobile ? "text-xs" : "text-sm"
                        )}>
                          {imageErrors.get(index)?.lastError || 'Image could not be loaded'}
                        </p>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {index !== selectedIndex && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => scrollTo(index === 0 ? 1 : index - 1)}
                        ...
                        {process.env.NODE_ENV === 'development' && performanceMetrics.get(index) && (
                          <details className="text-xs text-white/50 mt-2">
                            <summary className="cursor-pointer">Debug Info</summary>
                            <div className="mt-1 text-left">
                              <div>Retry attempts: {retryAttempts.get(index) || 0}</div>
                              <div>Error count: {imageErrors.get(index)?.count || 0}</div>
                              <div>Load time: {performanceMetrics.get(index)?.loadEnd ? 
                                `${performanceMetrics.get(index)!.loadEnd! - performanceMetrics.get(index)!.loadStart}ms` : 
                                'N/A'
                              }</div>
                            </div>
                          </details>
                  ...
                  {/* Retry Indicator */}
                  {retryAttempts.get(index) && (retryAttempts.get(index)! < maxRetryAttempts) && (
                    <div className="absolute top-4 right-4 bg-yellow-500/80 text-black px-2 py-1 rounded text-xs font-medium">
                      Retrying... ({retryAttempts.get(index)}/{maxRetryAttempts})
                    </div>
                  )}
                </div>
              </div>
